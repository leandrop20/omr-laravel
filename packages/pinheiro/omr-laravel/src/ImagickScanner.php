...
use ImagickDraw;
use Imagick;
use Pinheiro\OmrLaravel\Abstracts\Scanner;
...
class ImagickScanner extends Scanner
{
    ...
    protected function topRight(Point $near): Point
    {
        ...
        $last = new Point(
        	$x > $imagick->getImageWidth() ? $imagick->getImageWidth() : $x,
     ...
     * @param float $percent
     */
    protected function ajustSize(float $percent)
    {
    	$imagick = $this->getImagick();
     ...
     * @param float $degrees
     */
    protected function ajustRotate(float $degrees)
    {
    	if ($degrees < 0) {
            $degrees = 360 + $degrees;
    	}
    ...
    protected function rectangleArea(Point $a, Point $b, float $tolerance): Area
    {
        ...
        $pixels = $imagick->exportImagePixels(
        	$a->getX(),
        	$a->getY(),
        	$width,
        	$height,
     ...
     * @return Area
     */
    protected function circleArea(Point $a, float $radius, float $tolerance): bool
    {
        return true;
    }
