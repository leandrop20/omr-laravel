...
use Pinheiro\OmrLaravel\Result;
...
abstract class Scanner
{
     ...
     * @return Area
     */
    protected abstract function rectangleArea(Point $a, Point $b, float $tolerance);
     ...
     * @return Area
     */
    protected abstract function circleArea(Point $a, float $radius, float $tolerance);
     ...
     * @param float $percent
     */
    protected abstract function ajustSize(float $percent);
     ...
     * @param float $degrees
     */
    protected abstract function ajustRotate(float $degrees);
    ...
    public function scan(Map $map): Result
    {
        ...
        foreach($map->targets() as $target) {
            if ($target instanceof TextTarget) {
                $target->setImageBlob($this->textArea(
                		$target->getA()->moveX($ajustX)->moveY($ajustY),
                ...
                if ($target instanceof CircleTarget) {
                    $area = $this->circleArea(
                    	$target->getPoint()->moveX($ajustX)->moveY($ajustY),
     ...
     * @return Result
     */
    protected function createResult(string $imagePath): Result
    {
